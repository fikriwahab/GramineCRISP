#!/usr/bin/env python3
"""SQLite app benchmark under CRISP modes.

Workload: N independent INSERT transactions into SQLite database with
PRAGMA synchronous=FULL, so each commit issues fsync. Measures transactions
per second across CRISP modes (disabled, synchronous, optimistic, checker).

Light scope per app: 1 iter, 3 modes default, ~5-10 menit total.

Run on the SGX VM with:
    GRAMINE_CMD=gramine-sgx python3 c6a_sqlite.py --out microbench/sqlite.csv
"""
...
int main(int argc, char** argv) {
    ...
    if (sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS t (id INTEGER PRIMARY KEY, data TEXT)",
...
def build_app(workdir):
    ...
    subprocess.run(["gcc", "-O2", "-o", "app", "app.c", "-lsqlite3"], cwd=workdir, check=True,
...
def write_manifest(workdir, mode, mc_path, prob=None):
    ...
    manifest = MANIFEST_BASE.format(
        key=KEY, runtime=RUNTIME, arch_libdir=ARCH_LIBDIR,
...
def main():
    ...
    p.add_argument("--out", default="sqlite_results.csv", help="output CSV path")
    p.add_argument("--profile-out", default="sqlite_profile.csv", help="per-slot profile CSV path")
